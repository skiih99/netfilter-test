...
static u_int32_t print_pkt (struct nfq_data *tb)
{
	...
	if (http_flag) {
		int index = 0;
		while(1) {
			if (!memcmp(data + iphdr_len + tcphdr_len + index, "Host: ", 6)) {
				if (!memcmp(data + iphdr_len + tcphdr_len + index + 6, input_host, strlen(input_host))) {
					netfilter_flag = TRUE;
					break;
				} 
				else break;
...
int main(int argc, char **argv)
{
	...
	strncpy(input_host, argv[1], strlen(argv[1]));
